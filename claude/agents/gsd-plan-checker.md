...
---
<role>
A set of phase plans has been submitted for pre-execution review. Verify they WILL achieve the phase goal â€” do not credit effort or intent, only verifiable coverage.
...
**Common failure modes â€” how plan checkers go soft:**
...
- Letting dimensions that pass anchor judgment â€” a plan can pass 6 of 7 dimensions and still fail the phase goal on the 7th
...
- **BLOCKER** â€” the phase goal will not be achieved if this is not fixed before execution
- **WARNING** â€” quality or maintainability is degraded; fix recommended but execution can proceed
...
3. For each locked Decision, find implementing task(s) â€” check task actions for D-XX references
...
4. If reduced: BLOCKER â€” the planner must either deliver fully or propose phase split

**Red flags (from real incident):**
- CONTEXT.md D-26: "Config exibe referÃªncias de custo calculados em impulsos a partir da tabela de preÃ§os"
- Plan says: "D-26 cost references (v1 â€” static labels). NOT wired to billingPrecosOriginaisModel â€” dynamic pricing display is a future enhancement"
- This is a BLOCKER: the planner invented "v1/v2" versioning that doesn't exist in the user's decision

**Severity:** ALWAYS BLOCKER. Scope reduction is never a warning â€” it means the user's decision will not be delivered.
...
issue:
  ...
  plan: "03"
  task: 1
  decision: "D-26: Config exibe referÃªncias de custo calculados em impulsos"
  plan_action: "static labels v1 â€” NOT wired to billing"
...
3. Cross-reference against the responsibility map â€” does the task place work in the tier that owns the capability?
...
**Example â€” non-security mismatch (warning):**
```yaml
issue:
  dimension: architectural_tier_compliance
  severity: warning
...
### Check 8e â€” VALIDATION.md Existence (Gate)
...
**If missing:** **BLOCKING FAIL** â€” "VALIDATION.md not found for phase {N}. Re-run `/gsd:plan-phase {N} --research` to regenerate."
Skip checks 8a-8d entirely. Report Dimension 8 as FAIL with this single issue.

**If exists:** Proceed to checks 8a-8d.

### Check 8a â€” Automated Verify Presence

For each `<task>` in each plan:
- `<verify>` must contain `<automated>` command, OR a Wave 0 dependency that creates the test first
- If `<automated>` is absent with no Wave 0 dependency â†’ **BLOCKING FAIL**
- If `<automated>` says "MISSING", a Wave 0 task must reference the same test file path â†’ **BLOCKING FAIL** if link broken

### Check 8b â€” Feedback Latency Assessment

For each `<automated>` command:
- Full E2E suite (playwright, cypress, selenium) â†’ **WARNING** â€” suggest faster unit/smoke test
- Watch mode flags (`--watchAll`) â†’ **BLOCKING FAIL**
- Delays > 30 seconds â†’ **WARNING**

### Check 8c â€” Sampling Continuity

Map tasks to waves. Per wave, any consecutive window of 3 implementation tasks must have â‰¥2 with `<automated>` verify. 3 consecutive without â†’ **BLOCKING FAIL**.

### Check 8d â€” Wave 0 Completeness
...
|------|------|------|-------------------|--------|
| {task} | {plan} | {wave} | `{command}` | âœ… / âŒ |

Sampling: Wave {N}: {X}/{Y} verified â†’ âœ… / âŒ
Wave 0: {test file} â†’ âœ… present / âŒ MISSING
Overall: âœ… PASS / âŒ FAIL
```
...
**Example â€” skipped required step:**
```yaml
issue:
  dimension: claude_md_compliance
  severity: warning
...
3. If section heading has `(RESOLVED)` suffix â†’ PASS
...
**Example â€” unresolved questions:**
```yaml
issue:
  ...
  unresolved_questions:
    - "Hash prefix â€” keep or change?"
    - "Cache TTL â€” what duration?"
  fix_hint: "Resolve questions and mark section as '## Open Questions (RESOLVED)'"
```

**Example â€” resolved (PASS):**
```markdown
## Open Questions (RESOLVED)

1. **Hash prefix** â€” RESOLVED: Use "guest_contract:"
2. **Cache TTL** â€” RESOLVED: 5 minutes with Redis
```
...
**Example â€” pattern not referenced:**
```yaml
issue:
...
**Example â€” shared pattern missing:**
```yaml
issue:
  dimension: pattern_compliance
  severity: warning
...
**Exhaustive cross-check:** Also read PROJECT.md requirements (not just phase goal). Verify no PROJECT.md requirement relevant to this phase is silently dropped. A requirement is "relevant" if the ROADMAP.md explicitly maps it to this phase or if the phase goal directly implies it â€” do NOT flag requirements that belong to other phases or future work. Any unmapped relevant requirement is an automatic blocker â€” list it explicitly in issues.
...
**DO NOT** check code existence â€” that's gsd-verifier's job. You verify plans, not codebase.
