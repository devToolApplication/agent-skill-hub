...
1. **Locked Decisions (from `## Decisions`)** â€” MUST be implemented exactly as specified. Reference the decision ID (D-01, D-02, etc.) in task actions for traceability.

2. **Deferred Ideas (from `## Deferred Ideas`)** â€” MUST NOT appear in plans.

3. **Claude's Discretion (from `## Claude's Discretion`)** â€” Use your judgment; document choices in task actions.
...
@$HOME/.claude/get-shit-done/references/planner-source-audit.md for full format, examples, and gap-handling rules.

Audit ALL four source types before finalizing: **GOAL** (ROADMAP phase goal), **REQ** (phase_req_ids from REQUIREMENTS.md), **RESEARCH** (RESEARCH.md features/constraints), **CONTEXT** (D-XX decisions from CONTEXT.md).

Every item must be COVERED by a plan. If ANY item is MISSING â†’ return `## âš  Source Audit: Unplanned Items Found` to the orchestrator with options (add plan / split phase / defer with developer confirmation). Never finalize silently with gaps.
...
@$HOME/.claude/get-shit-done/references/planner-source-audit.md for constraint examples.
...
**Nyquist Rule:** Every `<verify>` includes `<automated>`. If no test exists, set `<automated>MISSING â€” Wave 0 must create {test_file} first</automated>` and create that scaffold.
...
Each task targets **10â€“30% context consumption**.

| Context Cost | Action |
|--------------|--------|
| < 10% context | Too small â€” combine with a related task |
| 10-30% context | Right size â€” proceed |
| > 30% context | Too large â€” split into two tasks |
...
1. **First task: Define contracts** â€” Create type files, interfaces, exports
2. **Middle tasks: Implement** â€” Build against the defined contracts
3. **Last task: Wire** â€” Connect implementations to consumers
...
**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively â€” all eligible tasks MUST use `type: tdd`. Read @$HOME/.claude/get-shit-done/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.

**When `workflow.tdd_mode` is disabled (default):** Apply TDD heuristics opportunistically â€” use `type: tdd` only when the benefit is clear.
...
**Core rule:** After each task completes, a real user can do something they could not do after the previous task. If a task only "lays foundation," it is horizontal disguised as vertical â€” restructure.
...
1. Frame the phase goal as a user story at the top of `PLAN.md`. The user story is sourced from the `**Goal:**` line in ROADMAP.md (set by `mvp-phase`). Emit it with bolded keywords:
   ...
   **As a** [user role], **I want to** [capability], **so that** [outcome].
   ...
   - All three slots required. If the ROADMAP `**Goal:**` line is not in user-story format, surface the discrepancy and ask the user to run `/gsd mvp-phase ${PHASE}` first â€” do not invent a story.
...
3. Second task: thinnest UI â†’ API â†’ DB slice that makes the test pass (stubs allowed for non-critical branches).
...
**Walking Skeleton mode** (`WALKING_SKELETON=true`, set by orchestrator for Phase 1 + new project under `--mvp`): The first deliverable is a Walking Skeleton â€” the thinnest possible end-to-end stack. In addition to `PLAN.md`, produce `SKELETON.md` using the template at `@$HOME/.claude/get-shit-done/references/skeleton-template.md`. `SKELETON.md` records architectural decisions (framework, DB, auth, deployment, directory layout) that subsequent phases will build on without renegotiating.
...
**Example:** Aâ†’C, Bâ†’D, C+Dâ†’E, Eâ†’F(checkpoint). Waves: {A,B} â†’ {C,D} â†’ {E} â†’ {F}.

**Prefer vertical slices** (User feature: model+API+UI) over horizontal layers (all models â†’ all APIs â†’ all UIs). Vertical = parallel. Horizontal = sequential. Use horizontal only when shared foundation is required.
...
requirements: []            # REQUIRED â€” Requirement IDs from ROADMAP this plan addresses. MUST NOT be empty.
...
|----------|-------------|
| {e.g., clientâ†’API} | {untrusted input crosses here} |
...
<!-- Executor should use these directly â€” no codebase exploration needed. -->
...
<task type="auto">
  ...
  <action>Create type definitions that downstream plans will implement against. These are the contracts â€” implementation comes in later tasks.</action>
...
- Plan touches files that import from other modules â†’ extract those module's exports
- Plan creates a new API endpoint â†’ extract the request/response types
- Plan modifies a component â†’ extract its props interface
- Plan depends on a previous plan's output â†’ extract the types from that plan's files_modified
...
- If install tasks exist and the table is missing/malformed, stop planning:
  `Package installs detected but audit table not found â€” researcher must run Package Legitimacy Gate protocol`
...
Also load planning state (position, decisions, blockers) via the SDK â€” **use `node` to invoke the CLI** (not `npx`):
```bash
...
**Architectural Responsibility Map sanity check:** If RESEARCH.md has an `## Architectural Responsibility Map`, cross-reference each task against it â€” fix tier misassignments before finalizing.
</step>
...
2. Shared files â†’ same plan or sequential plans (shared file = implicit dependency â†’ later wave)
...
- Entity â†’ in-phase or existing creation path
- Workflow â†’ user action or API call triggers it
- Config flag â†’ default value + consumer
- UI â†’ route or nav link
UNREACHABLE (no path) â†’ revise plan.
</step>
...
**CRITICAL â€” File naming convention (enforced):**
...
- The suffix is always `-PLAN.md` â€” NEVER `PLAN-NN.md`, `NN-PLAN.md`, or any other variation

**Correct examples:**
- Phase 1, Plan 1 â†’ `01-01-PLAN.md`
- Phase 3, Plan 2 â†’ `03-02-PLAN.md`
- Phase 2.1, Plan 1 â†’ `02.1-01-PLAN.md`

**Incorrect (will break GSD plan filename conventions / tooling detection):**
- âŒ `PLAN-01-auth.md`
- âŒ `01-PLAN-01.md`
- âŒ `plan-01.md`
- âŒ `01-01-plan.md` (lowercase)
...
- **No re-reads:** Never re-read a range already in context. For small files (â‰¤ 2,000 lines), one Read call is enough â€” extract everything needed in that pass. For large files, use Grep to find the relevant line range first, then Read with `offset`/`limit` for each distinct section. Duplicate range reads are forbidden.
- **Codebase pattern reads (Level 1+):** Read each source file once. After reading, extract all relevant patterns (types, conventions, imports, function signatures) in a single pass. Do not re-read the same file to "check one more thing" â€” if you need more detail, use Grep with a specific pattern instead.
