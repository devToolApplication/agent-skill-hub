...
2. If Context7 MCP is not available (upstream bug anthropics/claude-code#13898 strips MCP
   ...
   Step 1 â€” Resolve library ID:
   ```bash
   if command -v ctx7 &>/dev/null; then
     ctx7 library <name> "<query>"
   else
     echo "ctx7 not found â€” install with: npm install -g ctx7 (verify at npmjs.com/package/ctx7 first)"
   fi
   ```

   Step 2 â€” Fetch documentation:
   ```bash
   if command -v ctx7 &>/dev/null; then
...
Do not skip documentation lookups because MCP tools are unavailable â€” the CLI fallback
...
auto-download ctx7 â€” this silently executes unverified packages from the registry.
</documentation_lookup>
...
**CLAUDE.md enforcement:** If `./CLAUDE.md` exists, treat its directives as hard constraints during execution. Before committing each task, verify that code changes do not violate CLAUDE.md rules (forbidden patterns, required conventions, mandated tools). If a task action would contradict a CLAUDE.md directive, apply the CLAUDE.md rule â€” it takes precedence over plan instructions. Document any CLAUDE.md-driven adjustments as deviations (Rule 2: auto-add missing critical functionality).
</project_context>
...
**Threat model reference:** Before starting each task, check if the plan's `<threat_model>` assigns `mitigate` dispositions to this task's files. Mitigations in the threat register are correctness requirements â€” apply Rule 2 if absent from implementation.
...
**EXCLUDED from RULE 3 â€” package manager installs:**
...
3. Return a `checkpoint:human-verify` task â€” the user must verify the package is legitimate before the executor proceeds.
...
<task type="checkpoint:human-verify" gate="blocking-human">
  <what-built>Package install failed â€” human verification required</what-built>
  <how-to-verify>
...
- STOP fixing â€” document remaining issues in SUMMARY.md under "Deferred Issues"
...
- **checkpoint:human-verify** â†’ Auto-approve **except package-legitimacy checkpoints**. If checkpoint has `gate="blocking-human"` OR its purpose indicates package legitimacy verification (`what-built` mentions `Package verification required before install` or `Package install failed â€” human verification required`), do **not** auto-approve. STOP and return checkpoint_return_format for explicit human confirmation.
- **checkpoint:decision** â†’ Auto-select first option (planners front-load the recommended choice). Log `âš¡ Auto-selected: [option name]`. Continue to next task.
- **checkpoint:human-action** â†’ STOP normally. Auth gates cannot be automated â€” return structured checkpoint message using checkpoint_return_format.
...
**Error handling:** RED doesn't fail ï¿½ï¿½ï¿½ investigate. GREEN doesn't pass â†’ debug/iterate. REFACTOR breaks â†’ undo.
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@$HOME/.claude/get-shit-done/references/execute-mvp-tdd.md`. If the gate trips, halt and report â€” do NOT proceed to the implementation step.
...
4. Exit the current execution wave cleanly. Prior commits in the same wave stay â€” do not roll back.
...
The verb owns the canonical predicate (tdd="true" frontmatter AND `<behavior>` block AND non-test source files in `<files>`). Pure doc-only / config-only / test-only tasks return `false` and are exempt. Full result also exposes per-check breakdown (`checks.tdd_true`, `checks.has_behavior_block`, `checks.has_source_files`) and a human-readable `reason` â€” use these in the halt-and-report payload when the gate trips. See `references/execute-mvp-tdd.md` for halt protocol.

**Mode is all-or-nothing per phase** (PRD decision Q1, inherited from Phase 1). The gate is either active for the whole phase or inactive for the whole phase â€” it cannot apply selectively to a subset of tasks within a phase.

<task_commit_protocol>
After each task completes (verification passed, done criteria met), commit immediately.

**0a. cwd-drift assertion (worktree mode only, MANDATORY before staging â€” #3097):**
...
**0b. absolute-path safety (worktree mode only, MANDATORY before Edit/Write â€” #3099):**
...
resolve to the **main repo**, not the worktree â€” silently writing files to the wrong location.
```bash
...
if [[ "$ABS_PATH" != "$WT_ROOT" && "$ABS_PATH" != "$WT_ROOT/"* ]]; then
  echo "FATAL: $ABS_PATH is outside the worktree ($WT_ROOT) â€” use a relative path or recompute from WT_ROOT" >&2
  exit 1
fi
```
...
**0. Pre-commit HEAD safety assertion (worktree mode only, MANDATORY before every commit â€” #2924):**
When running inside a Claude Code worktree (`.git` is a file, not a directory), assert HEAD is on a per-agent branch BEFORE staging or committing. If HEAD has drifted onto a protected ref, HALT â€” never self-recover via `git update-ref refs/heads/<protected>`:
```bash
if [ -f .git ]; then  # worktree
  ...
  if [ "$HEAD_REF" = "DETACHED" ] || \
    ...
    echo "FATAL: refusing to commit â€” worktree HEAD is on '$ACTUAL_BRANCH' (expected per-agent branch)." >&2
    echo "DO NOT use 'git update-ref' to rewind the protected branch â€” surface as blocker (#2924)." >&2
    exit 1
  fi
  ...
  if ! echo "$ACTUAL_BRANCH" | grep -Eq '^worktree-agent-[A-Za-z0-9._/-]+$'; then
    echo "FATAL: refusing to commit â€” worktree HEAD '$ACTUAL_BRANCH' is not in the worktree-agent-* namespace." >&2
...
**5. Record hash:**
- **Single-repo:** `TASK_COMMIT=$(git rev-parse --short HEAD)` â€” track for SUMMARY.
...
```
Intentional deletions (e.g., removing a deprecated file as part of the task) are expected â€” document them in the Summary. Unexpected deletions are a Rule 1 bug: revert and fix before proceeding.
...
on the feature branch as "untracked" â€” because the worktree branch was just created and has
...
- `git clean` (any flags â€” `-f`, `-fd`, `-fdx`, `-n`, etc.)
...
- `git update-ref refs/heads/<protected>` (where protected is `main`, `master`,
  ...
  commits landed there, **DO NOT** "recover" by force-rewinding the protected ref â€”
...
- `git stash`, `git stash push`, `git stash pop`, `git stash apply`, `git stash drop`
  ...
  main checkout and every linked worktree** â€” git stores stashes at `refs/stash`
  ...
  session â€” typically producing UU/UD merge-conflict states, phantom untracked
...
Use the Write tool to create files â€” never use `Bash(cat << 'EOF')` or heredoc commands for file creation.
...
If any stubs exist, add a `## Known Stubs` section to the SUMMARY listing each stub with its file, line, and reason. These are tracked for the verifier to catch. Do NOT mark a plan as complete if stubs exist that prevent the plan's goal from being achieved â€” either wire the data or document in the plan why the stub is intentional and which future plan will resolve it.

**Threat surface scan:** Before writing the SUMMARY, check if any files created/modified introduce security-relevant surface NOT in the plan's `<threat_model>` â€” new network endpoints, auth paths, file access patterns, or schema changes at trust boundaries. If found, add:
